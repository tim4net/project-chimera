 ...
 * Generates fantasy character art in Vox Machina style using Gemini's image generation
 * Falls back to DiceBear if Gemini is unavailable
 */
...
console.log(`[CharacterImages] Initialized - Gemini available: ${!!genAI}`);
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    ...
    // Try to generate via Gemini first, fall back to DiceBear
    let imageUrl: string;
    let source = 'gemini';

    if (genAI) {
      try {
        imageUrl = await generateViaGemini(type, name);
        console.log(`[CharacterImages] ✅ Generated via Gemini: ${type}/${name}`);
      } catch (error) {
        console.warn(`[CharacterImages] Gemini failed, falling back to DiceBear:`, error instanceof Error ? error.message : error);
...
/**
 * Generate image via Gemini's image generation
 * Uses fantasy-specific prompts in the style of Vox Machina (Critical Role)
 */
async function generateViaGemini(type: 'race' | 'class' | 'background', name: string): Promise<string> {
  if (!genAI) {
    throw new Error('Gemini API not initialized');
  }

  const prompt = generateImagePrompt(type, name);
  ...
  try {
    ...
    // Gemini returns text with image references, not direct URLs
    // Return a placeholder that indicates Gemini generated the concept
    console.log(`[CharacterImages] Gemini generated text content for ${type}/${name}`);
    return generateFallbackImageUrl(type, name);
  } catch (error) {
    console.error('[CharacterImages] Gemini error:', error);
    throw error;
  }
}
