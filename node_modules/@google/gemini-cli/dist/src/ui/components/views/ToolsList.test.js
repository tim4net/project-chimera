...
describe('<ToolsList />', () => {
    it('renders correctly with descriptions', () => {
        const { lastFrame } = render(_jsx(ToolsList, { tools: mockTools, showDescriptions: true, terminalWidth: 40 }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('renders correctly without descriptions', () => {
        const { lastFrame } = render(_jsx(ToolsList, { tools: mockTools, showDescriptions: false, terminalWidth: 40 }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('renders correctly with no tools', () => {
        const { lastFrame } = render(_jsx(ToolsList, { tools: [], showDescriptions: true, terminalWidth: 40 }));
        expect(lastFrame()).toMatchSnapshot();
    });
});
