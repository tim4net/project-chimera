...
describe('Core System Prompt (prompts.ts)', () => {
    ...
    describe('with CodebaseInvestigator enabled', () => {
        ...
        it('should include CodebaseInvestigator examples in the prompt', () => {
            const prompt = getCoreSystemPrompt(mockConfig);
            expect(prompt).toContain("First, I'll use the Codebase Investigator to understand the current implementation");
            expect(prompt).toContain(`[tool_call: ${CodebaseInvestigatorAgent.name} for query 'Analyze the authentication logic`);
            expect(prompt).toContain("I'll use the Codebase Investigator to find the relevant code and APIs.");
            expect(prompt).toContain(`[tool_call: ${CodebaseInvestigatorAgent.name} for query 'Find the code responsible for updating user profile information`);
        });
    });
    describe('with CodebaseInvestigator disabled', () => {
        ...
        it('should include standard tool examples in the prompt', () => {
            const prompt = getCoreSystemPrompt(mockConfig);
            expect(prompt).not.toContain("First, I'll use the Codebase Investigator to understand the current implementation");
            expect(prompt).not.toContain(`[tool_call: ${CodebaseInvestigatorAgent.name} for query 'Analyze the authentication logic`);
            expect(prompt).toContain("First, I'll analyze the code and check for a test safety net before planning any changes.");
            expect(prompt).toContain("I'm not immediately sure how user profile information is updated. I'll search the codebase for terms like 'UserProfile'");
        });
    });
